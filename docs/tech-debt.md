...
| **AdminRiderView `ADMIN_NO = 1` 하드코딩 (FE)** | 2026-05-17 | `momoolggo2-fe/src/views/admin/AdminRiderView.vue:59` `const ADMIN_NO = 1  // TODO` — approveRider/suspendRider 호출 시 `approvedByAdminNo: 1`, `suspendedByAdminNo: 1` 고정 전송 | 작업 C/D 진입 시 admin 로그인 통합 후 — userStore.state.adminNo 또는 JWT claim에서 추출. 학원 발표 MVP 허용 범위(관리자 1명 단일 actor 가정). code-reviewer 작업 A 종결 검증 W-2 사례 (2026-05-17). |
| **`RedisRiderLocationStoreTest.getAll()` 단위 테스트 부재** | 2026-05-17 | `mmg-rider-service/.../RedisRiderLocationStoreTest.java` — save/get/delete 3건 unit 있고 getAll만 부재 | LocationServiceTest mock 검증으로 비즈니스 로직은 커버됨. Cursor 순회 → JSON 역직렬화 → Map 조립 직접 검증 부재. 통합 테스트(LocationIntegrationTest)는 Redis 의존이라 학원 환경 한정. 다음 R5 location 손볼 때 mock RedisTemplate.scan + Cursor mock으로 보강. code-reviewer Group 10 검증 W-1 (2026-05-17). |
...
| **라이더 제재 cross-schema 정합성 부재 — admin → auth `suspendUser` (user.status) vs rider §3.2 `suspend` (rider.status) 별 호출 (Q-A18 (b))** | 2026-05-17 | `mmg-admin-service/.../AdminUserController.suspendUser` (auth Feign 호출) + `mmg-admin-service/.../delivery/RiderApprovalController.suspend` (rider Feign 호출) — *별 endpoint*. admin 측 사용자가 라이더 제재 시 두 호출 모두 명시적 진행 필요. 트랜잭션 분리 위험 (one 성공 + one 실패 = 일관성 부재). | Phase 6+ outbox 패턴 또는 트랜잭션 분리 정합성 결정 — (A) admin → rider §3.2 호출 후 rider 측에서 auth Feign 호출 (rider 단독 처리) (B) admin → outbox 이벤트 발행 → auth + rider 양 측 listener (C) admin 측 두 Feign 호출 + compensation. Q-A18 (b) 결정 일관 — 작업 A 범위 좁힘. |
| **admin 외부 endpoint PATCH vs rider Internal Feign POST 분리 박제 (Q-A19 (다)) — 명명 컨벤션 혼동 가능성** | 2026-05-17 | `mmg-admin-service/.../delivery/RiderApprovalController` `@PatchMapping("/{riderNo}/approve\|suspend")` + `mmg-admin-service/.../feign/RiderFeignClient` `@PostMapping("/internal/rider/{riderNo}/approve\|suspend")` 분리 박제 (admin BlindController PATCH 패턴 일관 + interfaces.md §3.1/§3.2 POST 박제 추종) | Phase 6+ — 명명 컨벤션 ADR 검토. *외부 endpoint = PATCH (status 정정 RESTful 의미)* / *Internal Feign = POST (interfaces.md 박제)* 패턴 명문화. case-#33 후속 정정 누락 패턴 회피 = Q-A19 (다) 박제 인용 강제. |
| **interfaces.md §3.1 응답 `approvedAt` 박제 vs `RiderProfileRes` 실제 9 필드 차이 (Group 8.5 code-reviewer W-1, case-#33 변종)** | 2026-05-17 | `docs/adr/rider/interfaces.md:185` `"approvedAt": "2026-05-05T10:00:00"` 박제 vs `RiderProfileRes.java` 9 필드 (approvedAt 부재). FE 연동 시 approvedAt 참조 코드 null 수신 위험 | Group 8 FE 진입 전 또는 학원 발표 전 — (A) interfaces.md §3.1 응답 박제 정정 (approvedAt 제거, RiderProfileRes 9 필드 일관) 또는 (B) RiderProfileRes에 approvedAt 필드 추가 (BaseEntity.updatedAt 또는 별 audit log). 권장: (A) — Q-A18 (b) audit log Phase 6+ 위임 일관. |
| **RiderTest 전이 검증 실패 시나리오 + suspend 전체 단위 테스트 누락 (Group 8.5 W-3)** | 2026-05-17 | `mmg-rider-service/.../rider/model/RiderTest.java` — approve() PENDING happy 1건만. 누락 5건: approve_alreadyActive/eating/suspended throw + suspend_happy + suspend_alreadySuspended throw | Phase 6+ entity 단위 테스트 보강 — case-#38 강제 Service 단위 (RiderServiceTest 4건 Group 8.5 추가)와 별 트랙. entity 직접 호출 테스트는 Service 추상화 우회라 우선순위 낮음. 학원 발표 무관. |
| **admin-service 통합 테스트 0건 (Group 8.5 W-4, 기존 부채)** | 2026-05-17 | `mmg-admin-service` 전체 통합 테스트 부재. Group 8.5 신설 RiderApprovalController도 동일. Feign propagate 정책 실 동작 미검증. | Phase 6+ — admin-service 통합 테스트 인프라 도입 (Phase 4-A `InternalUserControllerIntegrationTest` 패턴 일관). 우선순위: 라이더 정산 + 라이더 관리 endpoint 최소 4건 (approve happy + Feign 500 propagate + suspend happy + settlement pending GET). team-handoff §11 (admin OkHttp timeout) 처리 시점 통합 검증. |
| **adminService.js #url 하드코딩 `http://localhost:8083/api/admin` (Group 8 Q-A23)** | 2026-05-17 | `momoolggo2-fe/src/services/adminService.js:4` `#url = 'http://localhost:8083/api/admin'`. httpRequester.js의 `axios.defaults.baseURL = VITE_API_BASE_URL` 사용 X. Gateway 경유 X (admin:8083 직접). 학원 발표 dev 환경 의도. | Phase 6+ — VITE_ADMIN_BASE_URL env 분리 또는 Gateway 경유 (`/api/admin` 라우트 추가). Group 8 신설 endpoint 6건 (rider list/approve/suspend + settlement 3)도 동일 패턴 박제. dev 환경 발표 후 prod 전환 시점 정정. |
| **AdminSidebar dead link 2건 (`/admin/user` `/admin/store` 라우터 미박제)** | 2026-05-17 | `momoolggo2-fe/src/components/admin/AdminSidebar.vue:9-10` 메뉴 항목 박제 + `src/router/index.js` 해당 라우트 0건. 사용자 클릭 시 빈 페이지. | Phase 6+ — (A) AdminSidebar 메뉴 항목 제거 또는 (B) 신설 (AdminUserView/AdminStoreView 박제). 기존 admin views 8건 패턴 일관 적용 시 단순. 학원 발표 시연 무관 (사용자가 미클릭하면). |
| **AdminDeliveryView monitor/notice 화면 활용 여부 미확정 (Q-A22 (나) 본 트랙 외)** | 2026-05-17 | `momoolggo2-fe/src/views/admin/AdminDeliveryView.vue` + `adminService.js getDeliveryMonitor/sendRiderNotice` 박제됨. 실제 화면 가동/시연 흐름 미검증. | Phase 6+ — AdminDeliveryView 화면 내용 검증 + 라이더 실시간 모니터링 + 공지 발송 흐름 가동 확인. 학원 발표 시연 가치 검토 후 정정/제거. Q-A22 (나) 결정 일관 (작업 A 범위 외). **2026-05-17 갱신**: Group 9 case-#37 발견 — AdminDeliveryView 기존 박제 (424 lines, R8 시점 추정) 확인. figma 매칭 80%+ (4 카운트/필터/검색/목록/공지 모달 ✅, 지도/라이더 코드 회수 ❌). Q-A29 (나) 검증 + tech-debt 등재만. |
| **AdminDeliveryView 네이버 지도 SDK 통합 부재 (Group 9 Q-A28 (다))** | 2026-05-17 | `momoolggo2-fe/src/views/admin/AdminDeliveryView.vue` 박제 — figma "실시간 배달 관제 구역" 지도 영역 박제 X. admin FE 지도 SDK 통합 0건 | Phase 6+ — 네이버 지도 SDK 통합 (작업 B `/api/map/key` endpoint 활용 가능). NaverMap rider FE R5 + main FE 회원가입 박제 패턴 인용. 학원 발표 시연 가치는 4 카운트 + 목록 + 공지 모달로 충분 (Q-A22 (나) + Q-A29 (나) 일관). dead config 회피 — 사용처 명확 후 진입. |
| **AdminDeliveryView "라이더 코드 회수" 버튼 부재 (Group 9 Q-A27 (나))** | 2026-05-17 | figma "관리자 라이더 관리.png" 좌하단 박제 — BE endpoint 박제 0건 + FE 박제 0건. 사용자 의도 미확정 (의미 모호 — 물품 회수/인증 코드/부정 사용 차단 후보) | Phase 6+ — 사용자 의도 확정 + BE endpoint 신설 결정 자리. 의미 모호 + α 추가 확장 비용 큼 → 본 트랙 외 (Q-A27 (나)). 작업 A FE 트랙 종결 시점 placeholder 미생성 (변경 0건 결정). |
